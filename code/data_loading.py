...
def get_unseen_data_loader( feature, label, cat_label, K ):
	...
	test_dataset = du.TensorDataset( test_feature, test_label, cat_test_label )
	test_loader = du.DataLoader( test_dataset, batch_size=test_feature.size(0))

	return train_loader, test_loader
'''
def get_observed_data_loader(feature, label, tissue_index_list, K, batch_size, tissue_num, fix_train_set_issue):
	...
	for tissue_index in train_tissue_index_list:
		...
		if(fix_train_set_issue):
			...
			#Select K samples from each lineage. This might make different K values less comparable.
